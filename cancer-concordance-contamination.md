...
This was present in somalier as the proportion of sites with AB > 0 and < 0.1 and AB < 1 and > 0.9, so sites that are nearly homozygous. This works well because it avoids looking at heterozygotes which are skewed by copy-number variation and loss-of-heterozygosity--an observation utilized by CHARR and CONPAIR. This is still available as `p_middling_ab` per-sample.
