...
when isMainModule:
  ...
  for v in vcf:
    ...
    if info.get("AF", afs) != Status.OK:
      continue
    if afs[0] < 0.05 or afs[0] > 0.95: continue
  ...
  for v in saved:
    ...
    if len(vs) > 0 and (v.v.closest(vs).start - v.v.start).abs < 5000:
      continue
